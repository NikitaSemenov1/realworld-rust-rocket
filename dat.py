...
def createStrExecuteInsert(nameTable, kwargs):
    for key in kwargs.keys():
        ...
        if "ARRAY[" in kwargs[key]:
            continue
        kwargs[key] = '\'' + kwargs[key] + '\''

    strExecute = "INSERT INTO " + NAME_SCHEMA + "." + nameTable + " (" + ", ".join(kwargs.keys()) + f") VALUES({','.join(kwargs.values())})"
...
def clearDataBase(con):
    ...
    tableList = ["users", "articles", "favorites", "follows", "comments"]
...
def fillArticles(con):
    ...
    for i in range(COUNT_OF_ARTICLES):
        ...
        dictOfExecute = {"slug":"name-of-the-article-number-" + str(i),
                         ...
                         "author":str(userIDList[i%(COUNT_OF_USERS-1)]),

                         "tag_list": "ARRAY['c++', 'cpp', 'python', 'ai', 'guid', 'backend', 'frontend', 'userver']"}
...
def fillFavorites(con):
    ...
    for userID in userIDList:
        ...
        for articleID in randArticleIDList:
            ...
            strExecute = createStrExecuteInsert("favorites", {"\"user\"":userID, "article": articleID})
...
def fillFollowers(con):
    ...
    for followed in userIDList:
        ...
        for follower in randFollowerList:
            ...
            strExecute = createStrExecuteInsert("follows", {"followed":followed, "follower": follower})
...
def fillComments(con):
    ...
    for i in range(COUNT_OF_COMMENTS):
        ...
        dictOfExecute = {"body":"О да!! Я восхищён этой бессмыслицей в кубе. Это всё явно показывает бессмысленость смысла",
                         ...
                         "author": userIDList[i%(COUNT_OF_USERS-1)],

                         "article":articleIDList[i%(COUNT_OF_ARTICLES-1)]}
...
def main():
    ...
    try:
        ...
        conn_string = "host=localhost port=5432 dbname=\'" + sys.argv[1] + "\' user=admin password=admin"
